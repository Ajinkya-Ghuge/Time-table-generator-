...
print("\nAll timetables have been exported to 'College_Timetables.xlsx'.")

# Merge cells for continued sessions
wb = openpyxl.load_workbook("College_Timetables.xlsx")
for sheet_name in wb.sheetnames:
    ...
    for r in range(2, ws.max_row + 1):
        ...
        while col <= ws.max_column:
            cell = ws.cell(r, col)
            if cell.value and ' (cont.)' in cell.value:
                prev_col = col - 1
                ws.merge_cells(start_row=r, start_column=prev_col, end_row=r, end_column=col)
                prev_cell = ws.cell(r, prev_col)
                prev_cell.alignment = openpyxl.styles.Alignment(horizontal='center', vertical='center')
                cell.value = None
                col += 1  # skip the merged column
            col += 1
wb.save("College_Timetablesajinkya.xlsx")